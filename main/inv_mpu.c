...
void mytest_write()//尝试初始化
{
    ...
    tmp=0x80;//复位
    ret= i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &tmp);
    printf("i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, %02X):%d\n",tmp,ret);
    ...
    tmp=(3<<3);
    ret= i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &tmp);
    printf("i2c_write(st.hw->addr, st.reg->gyro_cfg, 1,%02X):%d\n",tmp,ret);
    i2c_read(st.hw->addr, st.reg->gyro_cfg, 1, &data);
    printf("st.reg->gyro_cfg:%x\n",data);

    //MPU_Write_Byte(MPU_ACCEL_CFG_REG,fsr<<3);//设置加速度传感器满量程范围
    tmp=(0<<3);
    ret= i2c_write(st.hw->addr, st.reg->accel_cfg, 1, &tmp);
    printf("i2c_write(st.hw->addr, st.reg->accel_cfg, 1,%02X):%d\n",tmp,ret);
    i2c_read(st.hw->addr, st.reg->accel_cfg, 1, &data);
    printf("st.reg->accel_cfg:%x\n",data);

    //设置MPU6050的采样率(假定Fs=1KHz)
    //rate:4~1000(Hz)
    tmp=19;
    ret= i2c_write(st.hw->addr, st.reg->rate_div, 1, &tmp);
    printf("i2c_write(st.hw->addr, st.reg->rate_div, 1,&tmp):%d\n",data);
    i2c_read(st.hw->addr, st.reg->accel_cfg, 1, &data);
    printf("st.reg->accel_cfg:%x\n",data);
    ...
    //lpf:数字低通滤波频率(Hz)
    tmp=4;
    ret= i2c_write(st.hw->addr, st.reg->lpf, 1, &tmp);
    printf("i2c_write(st.hw->addr, st.reg->lpf, 1,&tmp):%d\n",data);
    i2c_read(st.hw->addr, st.reg->lpf, 1, &data);
    printf("st.reg->lpf:%x\n",data);
    ...
    //关闭FIFO
    i2c_write(st.hw->addr, st.reg->fifo_en, 1, &tmp);

    //INT引脚低电平有效
    i2c_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp);

    //器件ID
    ret=i2c_read(st.hw->addr, st.reg->who_am_i, 1, &data);
    printf("ret:%d\n",ret);
    if(ret==0x68)
    {
        //设置CLKSEL,PLL X轴为参考
        ...
        //加速度与陀螺仪都工作
        tmp=0x00;
        i2c_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &tmp);
...
void  temperature()//读取温度
{
    ...
    i2c_read(st.hw->addr, st.reg->temp, 2, buf_tmp);
    raw=((unsigned int)buf_tmp[0]<<8)|buf_tmp[1];  
